...
export function HeaderCells(props: TableHeaderCellsProps) {
    return props.columns.map((column: TableColumnProps, index: number) => {
        const cellProps = { ...column, columns: props.columns, rows: props.rows, data: props.data };
        return (
            <TableHeaderCell key={index} {...cellProps} />
        );
    });
}

export function HeaderActionCell(props: TableHeaderCellsProps) {
    return props.rows?.actions && <th {...getAttributes(props.rows?.actions?.attributes)}>{getRowActionsLabel(props)}</th>
}
...
export function DataCells(props: TableRowProps) {
    return props.columns && props.columns.map((column: TableColumnProps, index: number ) => (
        <TableDataCell key={index} row={props} column={column} />
    ));
}

export function RowActions(props: TableRowProps) {
    return props.actions && props.actions.items.map((action: TableRowActionProps, index: number) => {
